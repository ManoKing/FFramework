...
{
    internal class ObfuscatorConfigEditWindow
    {
        [MenuItem("编译选项/C#混淆设置")]
        static void Open()
        {
            ObfuscatorConfig config = AssetDatabase.LoadAssetAtPath<ObfuscatorConfig>(Const.ConfigAssetPath);
    ...
    internal class ObfuscatorConfigEditor : Editor
    {
        ...
        private string[] ObfuscateTypeStr = new string[] { "黑名单", "白名单", "黑白名单" };
        private string[] ObfuscateNameTypeStr = new string[] { "随机", "词库" };
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Space(10f);
            obfuscatorConfig.enableCodeObfuscator = EditorGUILayout.ToggleLeft("启用代码混淆", obfuscatorConfig.enableCodeObfuscator, EditorStyles.boldLabel);
            GUILayout.Space(10f);
            using (new EditorGUI.DisabledGroupScope(!obfuscatorConfig.enableCodeObfuscator))
            {
                ...
                {
                    EditorGUILayout.LabelField("随机种子", new GUILayoutOption[1] { GUILayout.Width(100f) });
                    obfuscatorConfig.randomSeed = EditorGUILayout.IntField(obfuscatorConfig.randomSeed);
                }

                obfuscatorConfig.useTimeSpan = EditorGUILayout.ToggleLeft("时间戳", obfuscatorConfig.useTimeSpan);

                EditorGUILayout.EndHorizontal();

                GUILayout.Space(5f);

                Header("混淆名称", obfuscatorConfig.enableNameObfuscate, (enable) => obfuscatorConfig.enableNameObfuscate = enable);
                using (new EditorGUI.DisabledGroupScope(!obfuscatorConfig.enableNameObfuscate))
                {
                    DrawLeft();
                    GUILayout.Space(5f);
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("过滤器", new GUILayoutOption[1] { GUILayout.Width(80f) });
                    ...
                    GUILayout.Space(5f);
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("混淆方式", new GUILayoutOption[1] { GUILayout.Width(80f) });
                ...
                Header("代码注入", obfuscatorConfig.enableCodeInject, (enable) => obfuscatorConfig.enableCodeInject = enable);
                using (new EditorGUI.DisabledGroupScope(!obfuscatorConfig.enableCodeInject))
                {
                    ...
                    EditorGUILayout.EndHorizontal();
                    GUILayout.Space(2f);
                    obfuscatorConfig.GarbageMethodMultiplePerClass = EditorGUILayout.IntField(new GUIContent("无用的方法生成倍数"), obfuscatorConfig.GarbageMethodMultiplePerClass);
                    GUILayout.Space(2f);
                    obfuscatorConfig.InsertMethodCountPerMethod = EditorGUILayout.IntField(new GUIContent("调用无用方法的次数"), obfuscatorConfig.InsertMethodCountPerMethod);
                    GUILayout.Space(5f);
                    DrawRight();
                }
            ...
            if (GUILayout.Button("浏览...", new GUILayoutOption[1] { GUILayout.Width(80f) }))
            {
            ...
            GUILayout.Space(5f);
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("输出路径", new GUILayoutOption[1] { GUILayout.Width(80f) });
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(5f);
            if (GUILayout.Button("混淆", new GUILayoutOption[1] { GUILayout.Height(60f) }))
            {
