...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceLoader
        {
            ...
            {
                var startTime = DateTime.UtcNow;
                Addressables.LoadAssetAsync<TextAsset>(binaryAssetName).Completed += result => {
                    float elapseSeconds = (float)(DateTime.UtcNow - startTime).TotalSeconds;
                    loadBinaryCallbacks.LoadBinarySuccessCallback(binaryAssetName, result.Result.bytes, elapseSeconds, userData);
                };
                /*
                ResourceInfo resourceInfo = GetResourceInfo(binaryAssetName);
                if (resourceInfo == null)
